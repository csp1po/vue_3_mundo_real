...
Já estamos familiarizados com a execução do comando ``dev`` para ativar nosso projeto em um host local. Nós o usamos ao longo dos tutoriais enquanto desenvolvemos o código do aplicativo. Já que estamos prontos para levar nosso projeto para o mundo real, podemos passar a usar o comando ``build``, que obviamente transforma nosso projeto em um produto utilizável que pode ser implantado. 

1.1 Agora, na linha de comando, digite o comando ``npm run build`` em nosso terminal (estando na pasta raiz do projeto). E observe o que acontece.
